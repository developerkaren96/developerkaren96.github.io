/*
 * GameCenterRTC — WebRTC DataChannel transport for GameCenter peers.
 *
 * Prefers direct peer-to-peer when the firewall/NAT allows it (no
 * relay-side bandwidth, lower latency, no per-message server load).
 * When the negotiated peer connection can't be established within a
 * timeout, transparently falls back to a relay-mediated socket path
 * (`_fallbackSocket`) so the peer connection still works — just over
 * the relay rather than direct.
 *
 * Constructor:
 *   - `_id`        : remote peer id.
 *   - `_socket`    : shared signalling/relay socket; used for SDP
 *                     exchange and as the fallback transport.
 *   - `_initiator` : whether *we* opened the offer (or are answering).
 *   - `_parent`    : the GameCenter session that owns this RTC.
 *
 * The full file (below) wires up:
 *   - simple-peer (`new SimplePeer(...)`) signalling exchange.
 *   - `signal` / `connect` / `data` / `close` handlers.
 *   - `fallbackToSocket()` — abandons the peer and switches to
 *     relay-mediated send, telling the remote peer to do the same.
 *
 * v2 sibling: GameCenterRTC2 (0218).
 */
